...
function checkRandomAnswer() {
  if (props.detailAnswer.randomAnswer) {
    randomListAnswer.value = props.detailAnswer.randomAnswer
  } else {
    randomListAnswer.value = shuffle(props.detailAnswer)
    emit('randomQuiz', randomListAnswer.value)
  }
}
...
<template>
  ...
  >
    <template v-for="(answer, index) in randomListAnswer" :key="answer">
      <div
